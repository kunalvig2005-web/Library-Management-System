...
class Main:
    def __init__(self):
        ...
        self.mainLabel = Label(self.root, text="View Students", font=('Arial', 26, 'bold'),bg="#D9CBA0",fg='#4B3D2D')
        ...
        self.searchBtn = Button(self.searchFrame, text="Search", font=self.font,command=self.search_students,bg="#D9CBA0")
        ...
        self.student_table=ttk.Treeview(self.root,show='headings',columns=['id','name','email','mobile','gd'])
        self.student_table.pack(pady=20,expand=True,fill='both',padx=30)
        self.student_table.heading('id',text='ID')
        self.student_table.heading('name',text='Name')
        self.student_table.heading('email',text='Email')
        self.student_table.heading('mobile',text='Mobile')
        self.student_table.heading('gd',text='gender')
        ...
        self.student_table.column('mobile', anchor='center')
        self.student_table.column('gd', anchor='center')

        style = ttk.Style()
        style.configure('Treeview', font=self.font, rowheight=40)
        style.configure('Treeview.Heading', font=self.font,foreground='#4B3D2D')

        self.student_table.bind("<Double-1>", self.openupdatewindow)
    ...
    def getValues(self):
        q = "select id, name, email, mobile, gender from students"
        self.cr.execute(q)
        ...
        index = 0
        for i in result:
            self.student_table.insert('', index=index, values=i)
            index += 1
    def search_students(self):
        text = self.txt0.get()
        q0 = f"select id, name, email, mobile, gender from students where name like '%{text}%' or email like '%{text}%' or mobile like '%{text}%' or gender like '%{text}&'"
        self.cr.execute(q0)
        result=self.cr.fetchall()
        for item in self.student_table.get_children():
            self.student_table.delete(item)
    ...
    def openupdatewindow(self,event):
        id=self.student_table.selection()[0]
        metadata=self.student_table.item(id)
        ...
        self.mainLabel2 = Label(self.root1, text="Update/Delete Students", font=('Arial', 24, 'bold'),bg="#D9CBA0",fg='#4B3D2D')
        ...
        self.lb1 = Label(self.formFrame, text="Student ID", font=self.font,fg='white',bg="#A67C4D")
        ...
        self.lb5 = Label(self.formFrame, text="Select Gender", font=self.font,fg='white',bg="#A67C4D")
    ...
    def updateadmin(self):
        ...
        if name == "" or email == "" or mobile == "" or gender == "":
            msg.showwarning("Warning", "Please fill all fields", parent=self.root1)
        else:
            if verifyMobile(mobile) == True and verifyEmail(email) == True:
                q1 = f"select * from students where email='{email}' or mobile='{mobile}'"
                self.cr.execute(q1)
                result = self.cr.fetchall()
                print(result)
                if len(result)<=1:
                    q2 = f"update students set name='{name}', email='{email}', mobile='{mobile}', gender='{gender}' where id={id}"
                    self.cr.execute(q2)
                    self.conn.commit()
                    self.root1.destroy()
    ...
    def deleteadmin(self):
        id = self.txt1.get()
        q4 = f"delete from students where id='{id}'"
        self.cr.execute(q4)
        self.conn.commit()
        self.root1.destroy()
